...
# 42 Webserv Evaluation Tests - Stress Testing with Siege
...
if ! command -v siege &> /dev/null; then
    echo -e "${RED}❌ Siege is not installed${NC}"
    echo "Install with: brew install siege"
    exit 1
fi

echo -e "${BLUE}🔧 Installing/Checking Siege...${NC}"
echo "Siege version: $(siege --version 2>&1 | head -1)"

# Create simple test page for stress testing
mkdir -p ../tmp/www
echo "<html><body><h1>Stress Test Page</h1><p>Simple content for load testing</p></body></html>" > ../tmp/www/stress_test.html
